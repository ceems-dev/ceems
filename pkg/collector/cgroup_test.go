...
func TestNewCgroupManagerV1(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			manager: openpbs,
			mountPoints: []string{
				"testdata/sys/fs/cgroup/cpu,cpuacct/pbs_jobs.service",
			},
			numCgroups: 3,
		},
		{
...
func TestCgroupsV1Metrics(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			manager: slurm,
			expected: cgMetric{
				cgroup:          cgroup{uuid: "1009248", path: cgroupPath{rel: "/slurm/uid_1000/job_1009248"}},
				cpuUser:         0.39,
				cpuSystem:       0.45,
		...
		},
		{
			...
			manager: openpbs,
			expected: cgMetric{
				cgroup:          cgroup{uuid: "1009249[1].testhost-1", path: cgroupPath{rel: "/pbs_jobs.service/jobid/1009249[1].testhost-1"}},
				cpuUser:         2483.49,
				cpuSystem:       0.1,
				cpuTotal:        2483.616433633,
				cpus:            5000,
				cpuPressure:     0,
				memoryRSS:       1.0420224e+07,
				memoryCache:     0,
				memoryUsed:      1.5839232e+07,
				memoryTotal:     9.223372036854772e+18,
				memoryFailCount: 0,
				memswUsed:       0,
				memswTotal:      0,
