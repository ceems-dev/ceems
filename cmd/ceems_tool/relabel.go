...
func CreatePromRelabelConfig(
	...
	for _, job := range activeJobs {
		...
		default:
			for _, metricName := range jobSeries[job] {
				if strings.Contains(string(metricName), gpuSeries[2]) {
	...
	err = yamlEncoder.Encode(&PromConfig{ScrapeConfigs: scrapeConfigs})
	if err != nil {
	...
	// Starting from v0.11.0, there is no need to set up these relabel configs as recording rules
	// will take care of it. Inform that to users
	fmt.Fprintln(os.Stderr, "WARNING:")
	fmt.Fprintln(os.Stderr, "Starting from v0.11.0 the relabelling of metrics is handled by recording rules generated by ceems_tool")
	fmt.Fprintln(os.Stderr, "There is no need to add the generated relabel_configs to Prometheus' scrape configs")
